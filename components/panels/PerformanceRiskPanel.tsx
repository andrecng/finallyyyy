...
export default function PerformanceRiskPanel({ out }: { out: any }) {
  ...
  const rows: { label: string; val: any; kind: "pct" | "num" }[] = [
    ...
    { label: "Max pertes consécutives", val: k.max_consecutive_losses, kind: "num" },
    { label: "Best day", val: k.best_day_return, kind: "pct" },
    { label: "Worst day", val: k.worst_day_return, kind: "pct" },
    { label: "Jours recov. après max DD", val: k.recovery_days_maxdd, kind: "num" }
  ];
